...
def compass_to_rgb(hue, saturation=1., value=1.):
    h = float(hue)
    s = float(saturation)
    v = float(value)
    h60 = h / 60.0
...
def get_color_from_note(note, saturation=1., value=1.):
    if note == -1:
        return (0.9, 0.9, 0.9)
    hue = 360. * ((note * 7) % 12) / 12.
    return compass_to_rgb(hue, saturation, value)
